...
- `BBox.cu/.cuh` - Axis-aligned bounding box operations
...
### Release Mode Configuration Fixes
- **Fixed**: Release mode include paths now include CUDA samples Common directory
- **Fixed**: Release mode library paths corrected
- **Fixed**: Collision detection robustness - Removed `USE_EPSILON_TEST` conditional, EPSILON checks always enabled
- **Important**: Use Debug GLFW library for Release builds too (until GLFW Release build is available)
...
### Important Fixes
- **Data Race Issue Fixed**: `_TimeCost` was changed from static to non-static member variable
  - **Problem**: Two `CudaBVH` objects created per frame were concurrently modifying the same static array
  - **Solution**: Each object now has its own `_TimeCost` instance
  - **Result**: Accurate timing for both objects (nut and wrench)
...
// Build model matrix with time-based transformation
...
- **Window resize now handled**: Dynamic viewport and projection matrix updates
- **Continuous refresh**: Window remains responsive during resize/move operations
- **Memory layout guaranteed**: All interop classes use #pragma pack and static_assert
- **Release mode collision detection fixed**: EPSILON robustness checks always enabled
...
### Release Mode Collision Detection
- **Problem**: Release mode wasn't detecting collisions due to missing `USE_EPSILON_TEST` macro
- **Fix**: Removed conditional compilation, EPSILON checks are always enabled
- **Impact**: Collision detection now works correctly in both Debug and Release modes

### Memory Alignment Safety
- **Problem**: CUDA-OpenGL interop with custom classes could have padding issues
- **Fix**: Added `#pragma pack(push, 1)` and `static_assert` to Point, Triangle, BBox classes
- **Impact**: Compile-time validation of memory layout, no runtime surprises

### Release Mode Build Configuration
- **Problem**: Release mode missing CUDA samples include path and wrong GLFW library path
- **Fix**: Updated .vcxproj to include CUDA samples and correct library paths
- **Impact**: Release mode now compiles successfully
